...
When("el usuario crea un grupo indicando el nombre {string}", async ({ assembly }, nombre) => {
    await assembly.grupos.crearCon(nombre);
});

When("el usuario crea un grupo indicando que sus miembros son {string} y {string}", async ({ assembly }, miembro1, miembro2) => {
...
When("el usuario intenta crear un grupo indicando un único miembro", async ({ assembly }) => {
    await assembly.grupos.crearConUnUnicoMiembro();
})

Then("debería ser informado que necesita tener al menos dos miembros", async ({ assembly }) => {
...
Then('debería visualizar dentro del listado el grupo con el nombre indicado', async ({ assembly }) => {
    await assembly.grupos.validarNombreDeGrupo();
});

Then('visualiza dentro del listado el grupo con los miembros indicados', async ({ assembly }) => {
