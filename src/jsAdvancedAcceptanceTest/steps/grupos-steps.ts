...
When(
  ...
  async ({ assembly }, nombre) => {
    await assembly.grupos.crearGrupo(generarNombreUnico(nombre),["Victor", "Brenda"]);
  }
);
...
When("el usuario crea un grupo", async ({ assembly }) => {
  await assembly.grupos.crearGrupo(generarNombreUnico("Grupo de 4"),["Guido", "Laura", "Mariano", "Juan Cruz"]);
});
